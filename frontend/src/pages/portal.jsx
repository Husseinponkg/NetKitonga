...
function Portal() {
  ...
  const resolveRouterFromIp = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/routers/resolve?ip_address=${encodeURIComponent(routerIp)}`
      );
  ...
  const fetchActivePackages = async () => {
    try {
      ...
      showMessage(`Could not retrieve packages: ${error.message}`, "error");
    }
  };
  ...
  const handlePayAndConnect = async (e) => {
    ...
    if (!selectedPackageId || !phoneNumber.trim()) {
      showMessage("Please select a package and enter your mobile-money number.", "error");
      setIsLoading(false);
      return;
    }

    if (!effectiveTenantId || !effectiveBranchId || !effectiveRouterId) {
      showMessage("This payment portal is missing the tenant, branch, or router configuration.", "error");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!buyerResponse.ok)
        throw new Error(buyerResult.detail || "Could not register this device.");
      ...
      showMessage("Contacting AzamPay...", "success");
      ...
      if (response.ok) {
        const gatewayMessage =
          result?.message ||
          "AzamPay accepted the request. Please check your phone and enter your mobile money PIN to complete payment.";
        showMessage(`✨ ${gatewayMessage}`, "success");
      } else if (response.status === 504) {
        showMessage("AzamPay did not respond in time. Please try again or contact support.", "error");
      } else {
        const detail =
          typeof result === "string" ? result : result.detail || JSON.stringify(result);
        showMessage(`Checkout rejected: ${detail}`, "error");
      }
    } catch (error) {
      const message =
        error.message ||
        "Failed to establish communications with the billing server platform.";
  ...
  const handleVoucherActivation = async (e) => {
    ...
    showMessage("Validating your token code voucher...", "success");
    // TODO: integrate voucher activation API
  };
  ...
  const getSelectedPackage = () =>
    packageCatalog.find((pkg) => pkg.id === selectedPackageId);
  ...
  return (
    <div className="portal-root">
      ...
      <div className="portal-card">
        ...
        {/* Message */}
        {uiMessage && <div className={`message-box ${uiMessageType}`}>{uiMessage}</div>}
          ...
          <form onSubmit={handlePayAndConnect}>
            <div style={{ marginBottom: "10px" }}>
              ...
              {packageCatalog.length === 0 ? (
                ...
                >
                  <span
                    style={{ display: "block", fontSize: "1.8rem", marginBottom: "6px" }}
                  >
                    ⏳
                  </span>
                  Loading packages...
                    ...
                    <div className="selected-summary">
                      ...
                      <div>
                        ...
                        <div className="summary-price">
                          TZS {Number(selectedPackage.price).toLocaleString()}
                        </div>
                      </div>
                    </div>
                  )}
